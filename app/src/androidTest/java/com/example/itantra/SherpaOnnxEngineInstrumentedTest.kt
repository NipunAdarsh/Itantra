 ...
 * won't pass without real speech; the TTS half should be audible.
 */
...
class SherpaOnnxEngineInstrumentedTest {
    ...
    fun liveEndToEndPipeline() {
        ...
        try {
            // --- STT half: mic -> MicAudioCapture -> SileroVoiceActivityDetector -> recognizer ---
            ...
            // --- TTS half: tts.generate() -> AudioTrackPlayer (audible) ---
            engine.synthesizeAndPlay("Phase four integration test successful")
            runBlocking { delay(3_000) } // synthesizeAndPlay is fire-and-forget; give it time to finish
        } finally {
            engine.release()
        }
    }
