...
class TenLanguageBenchmarkTest {
    ...
    @Test
    fun runFullTenLanguageBenchmark() {
        val context = InstrumentationRegistry.getInstrumentation().targetContext
        Log.i(TAG, "=================================================================")
        Log.i(TAG, "STARTING 10-LANGUAGE EMPIRICAL BENCHMARK (30 Ground Truth Tests)")
        Log.i(TAG, "=================================================================")
        ...
        try {
            ...
            for ((idx, test) in testCorpus.withIndex()) {
                ...
                // 2. Synthesize audio waveform for ground truth
                val audioFloatArray = generateAudioForText(context, nativeTts, engine, test.groundTruth, test.language)
                ...
                // 4. Calculate WER, CER, RTF, Accuracy
                ...
                // 5. Test TTS Engine for this language
            ...
            val resultFile = File(context.filesDir, "benchmark_results.json")
            resultFile.writeText(resultsArray.toString(2))
            Log.i(TAG, "Benchmark results written to: ${resultFile.absolutePath}")
            Log.i(TAG, "BENCHMARK_JSON_OUTPUT_START\n${resultsArray.toString(2)}\nBENCHMARK_JSON_OUTPUT_END")
    ...
    private fun evaluateTTS(
        ...
        return when (lang) {
            AppLanguage.ENGLISH, AppLanguage.HINDI -> {
                ...
                try {
                    // Piper neural TTS synthesis check
    ...
    private fun generateAudioForText(
        ...
        // 1. If English or Hindi, synthesize via Piper VITS
        ...
        // 2. Synthesize via Android Native TTS to a temporary 16kHz WAV file
        if (nativeTts != null) {
            val locale = Locale.forLanguageTag("${lang.code}-IN")
            nativeTts.language = locale
        ...
        // Fallback: Generate clean synthetic acoustic waveform corresponding to phonetic duration
        val numSamples = 16000 * 3 // 3 seconds
    ...
    // ── ERROR RATE COMPUTATION UTILITIES ──────────────────────────────────────
